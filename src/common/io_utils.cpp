...
void CubeUsage(const char *exec) {
    fprintf(stderr, "\nUsage: %s \n"
                        "[+] -d [input_file]\n"
                        "[+] -q [query_file]\n"
...
std::string USER_PromtExit(const std::string &message) {
...
void LSH_parse(int argc, char * const argv[], LSH_Args **args) {
...
void Cube_Parse(int argc, char * const argv[], CubeArgs **args) {
    ...
    *args = new CubeArgs(dataset_file, query_file, output_file, nn_num, rad, projection_dimension, max_candidates, max_probes); 
}


void user_interface(CubeArgs **args) {
    ...
    if (*args == nullptr) {
        ...
        *args = new CubeArgs(input_file, query_file, output_file);
    }
}
...
uint32_t BigEND_LittleEND(uint32_t big_endian) {
...
void PrintStats(const uint16_t nns, const size_t size, const std::vector<std::vector<std::pair<uint32_t, size_t>>> &ann_res, \
