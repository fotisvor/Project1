...
class LSH {
    ...
    private:
        ...
        std::vector<Amplified_HASH_function<T>> g_hash_functions;
    ...
    public:
         ...
         LSH(    const uint16_t &L, const uint16_t &N, \
            ...
            for (size_t i = 0; i != L; ++i) {
                g_hash_functions.emplace_back(Amplified_HASH_function<T>(K, D, m, M, w));
            }
            ...
            for (size_t i = 0; i != L; ++i) {                
                ...
                for (size_t index = 0; index != n_vectors; ++index) {           
                    amplified_value = g_hash_functions[i].amplfied_FunctionConstructor(dataset[index]);
        ...
        std::vector<std::pair<uint32_t, size_t>> approximate_KNN(const std::vector<T> &query) {
            ...
            for (size_t i = 0; i != L; ++i) {
                af_value = g_hash_functions[i].amplfied_FunctionConstructor(query);
                ...
                for (auto const &index : bucket) {
                    ++checked;
                    dist = euclidean_distance_rd<T> (dataset[index], query);
                    if (dist < min_dist) {
                        min_dist = dist;
        ...
        std::vector<size_t> approximate_range_search(const double &c, const double &r, const std::vector<T> &query) {
            ...
            for (size_t i = 0; i != L; ++i) {
                ...
                for (auto const &index : bucket) {
                    if (euclidean_distance_rd<T> (dataset[index], query) < (c * r)) {
